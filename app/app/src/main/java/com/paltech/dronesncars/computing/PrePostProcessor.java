...
public class PrePostProcessor {
     ...
     */
    static ArrayList<ScanResult> nonMaxSuppression(ArrayList<ScanResult> boxes, int limit, float threshold) {
        ...
        ArrayList<ScanResult> selected = new ArrayList<>();
        ...
        for (int i = 0; i < boxes.size() && !done; i++) {
            if (active[i]) {
                ScanResult boxA = boxes.get(i);
                selected.add(boxA);
                if (selected.size() >= limit) break;

                for (int j = i + 1; j < boxes.size(); j++) {
                    if (active[j]) {
                        ScanResult boxB = boxes.get(j);
    ...
    public static ArrayList<ScanResult> outputsToNMSPredictions(float[] outputs, float imgScaleX, float imgScaleY, float ivScaleX, float ivScaleY, float startX, float startY) {
...
//                Rect rect = new Rect((int) (startX + ivScaleX * left), (int) (startY + top * ivScaleY), (int) (startX + ivScaleX * right), (int) (startY + ivScaleY * bottom));
                ...
                ScanResult result = new ScanResult(mClasses[cls], outputs[i * 7 + 4], rect);
                results.add(result);
            }
        }
